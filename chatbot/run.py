...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_wrong_answer_num", default=15, type=int)
    parser.add_argument("--val_wrong_answer_num", default=63, type=int)
    ...
    MODEL_CLASSES = {
        'bert': (RobertaConfig, RobertaTokenizerFast, RobertaModel),
    }
    ...
    if not args.eval:
        train_dataset = SelectionDataset(os.path.join(args.train_dir, args.train_file),
                                                                      context_transform, response_transform, concat_transform, mode=args.architecture, num=args.train_wrong_answer_num)
        val_dataset = SelectionDataset(os.path.join(args.train_dir, args.dev_file),
                                                                  context_transform, response_transform, concat_transform, mode=args.architecture, num=args.val_wrong_answer_num)
    ...
    bert_config = ConfigClass.from_pretrained(model_name)
    ...
    if args.use_pretrain and not args.eval:
        ...
        bert = BertModelClass.from_pretrained(model_name, config=bert_config, state_dict=model_state_dict)
        del model_state_dict
    else:
        bert = BertModelClass.from_pretrained(model_name, config=bert_config)
